function [asymptotic_rates, fixation_time] = asymptotic_convex_hull(grid, values, rates, asymptotic_rates, fixation_time)
    ...
    hull_indices = zeros(length(grid));
    ...
    while true
        ...
        fixation_time = max(fixation_time, maxval_except(  (   (values(hull_indices(j+1))  - values(hull_indices(j)))  ...
                                                             / (grid(hull_indices(j+1))    - grid(hull_indices(j)))    ...
                                                             - (values(hull_indices(j)+1:end) - values(hull_indices(j)))  ...
                                                             / (grid(hull_indices(j)+1:end)   - grid(hull_indices(j)))    ...
                                                           )                                                           ...
                                                         / (   (rates(hull_indices(j)+1:end)  - rates(hull_indices(j)))   ...
                                                             / (grid(hull_indices(j)+1:end)   - grid(hull_indices(j)))    ...
                                                             - (rates(hull_indices(j+1))   - rates(hull_indices(j)))   ...
                                                             / (grid(hull_indices(j+1))    - grid(hull_indices(j)))    ...
                                                           ), except=hull_indices(j+1)-hull_indices(j)));
        j = j + 1;
